...
test("routes room-server failures into the terminal transcript", async () => {
  ...
  assert.match(source, /rejectInputToTerminal\(data\.message \|\| "That room didn't come together\. Try again in a moment\."\)/);
  assert.match(source, /rejectInputToTerminal\("The room server is taking a breather\. Try again in a moment\."\)/);
...
test("keeps invalid-input stagger from replaying the composer slide-up", async () => {
  const source = await readFile(new URL("../components/direction-two-shell.tsx", import.meta.url), "utf8");
  const frameClassName = source.match(/className=\{`direction-two-terminal-frame[^`]*`\}/)?.[0] ?? "";

  assert.notEqual(frameClassName, "", "expected the landing terminal frame className template");
  assert.match(frameClassName, /isInputNudging \? "direction-two-input-nudge /);
  assert.doesNotMatch(frameClassName, /direction-two-composer-entry/);
