...
test("expands the room composer while the style command is active", () => {
  assert.deepEqual(
    getRoomComposerChrome({
      composerStatus: { tone: "muted", message: "style: 1 orange, 2 blue, 3 crimson, 4 purple, 5 surprise" },
...
test("keeps the idle room composer aligned to the landing composer anchor", async () => {
  const source = await readFile(new URL("../app/room/[id]/page.tsx", import.meta.url), "utf8");

  assert.match(source, /const composerHasTopContent = passwordReveal \|\| composerExpanded \|\| showSlashSuggestions;/);
  assert.match(source, /minHeight: composerHasTopContent \? "76px" : "58px"/);
  assert.match(source, /padding: composerHasTopContent \? "10px 12px" : routeComposerGeometry\.framePadding/);
});
...
test("gives the room composer a sharp, theme-aware terminal frame", async () => {
  ...
  assert.match(source, /padding: `0 \$\{routeComposerGeometry\.horizontalPadding\} \$\{routeComposerGeometry\.bottomPadding\}`/);
});
