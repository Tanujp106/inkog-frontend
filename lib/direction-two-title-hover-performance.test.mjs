...
test("follows the pointer directly while keeping a smooth return transition", () => {
  ...
  const basePixelStart = stylesSource.indexOf(".direction-two-mark-pixel-active {");
  const basePixelEnd = stylesSource.indexOf("}", basePixelStart);
  const basePixelBlock = stylesSource.slice(basePixelStart, basePixelEnd);

  assert.doesNotMatch(activeHoverBlock, /transition:\s*transform/);
  assert.match(activeHoverBlock, /transition:\s*[\s\S]*background-color var\(--direction-two-title-hover-highlight-duration/);
  assert.match(basePixelBlock, /transform var\(--direction-two-title-magnet-return-duration/);
});

test("uses a bright static highlight instead of a hover shimmer animation", () => {
  const highlightStart = stylesSource.indexOf(
    '.direction-two-mark[data-mark-phase="interactive"] .direction-two-mark-pixel-active[data-mark-magnet-highlight="true"]',
  );
  const highlightEnd = stylesSource.indexOf("}", highlightStart);
  const highlightBlock = stylesSource.slice(highlightStart, highlightEnd);

  assert.doesNotMatch(stylesSource, /@keyframes direction-two-mark-hover-shimmer/);
  assert.match(highlightBlock, /background-color: color-mix/);
  assert.match(highlightBlock, /filter: brightness\(var\(--direction-two-title-hover-highlight-brightness/);
  assert.match(highlightBlock, /box-shadow:/);
  assert.match(
    stylesSource,
    /\.direction-two-mark\[data-mark-phase="interactive"\] \.direction-two-mark-pixel-active\[data-mark-magnet-highlight="true"\]/,
  );
...
test("wires title hover duration, delay, and easing dials into the live style pipeline", () => {
  ...
  assert.match(stylesSource, /var\(--direction-two-title-hover-easing, var\(--ease-out-strong\)\)/);
});
