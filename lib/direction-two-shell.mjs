...
export function createDirectionTwoGuidedCommandSegments(command = "/create") {
  return [{ id: "command", value: command }];
}

export function appendDirectionTwoGuidedCommandSegment(segments, id, value) {
  return updateDirectionTwoGuidedCommandSegment(segments, id, value);
}

export function updateDirectionTwoGuidedCommandSegment(segments, id, value) {
  const nextSegment = { id, value };
  const existingIndex = segments.findIndex(segment => segment.id === id);

  if (existingIndex === -1) return [...segments, nextSegment];

  return segments.map((segment, index) => (index === existingIndex ? nextSegment : segment));
}

export function serializeDirectionTwoGuidedCommandSegments(segments) {
...
const directionTwoThemePalette = [
  ...directionTwoThemes,
...
export function getDirectionTwoSlashCommandSuggestions(value) {
  ...
  return commands
    ...
    .map(command => ({
      id: command,
      command: `/${command}`,
      title: slashCommandTitles[command],
...
export function getDirectionTwoPasswordMask(value, visibleIndex = value.length - 1) {
  if (!value) return "";

  const index = Math.min(Math.max(visibleIndex, 0), value.length - 1);
  return `${"*".repeat(index)}${value[index]}${"*".repeat(value.length - index - 1)}`;
}
...
export function getDirectionTwoCreateAnswerError(step, rawAnswer) {
  ...
  switch (step) {
    case "topic":
      return answer ? null : "Give your room a name first.";
    case "expiry": {
      const expiry = Number(answer);
      return Number.isFinite(expiry) && expiry >= 15 ? null : "Room time needs to be at least 15 minutes.";
    }
    case "limit": {
      const roomLimit = Number(answer);
      return Number.isInteger(roomLimit) && roomLimit >= 1 && roomLimit <= 30
        ? null
        : "Choose a member limit from 1 to 30.";
    }
    case "password-choice":
      return noPasswordAnswers.has(answer.toLowerCase()) || yesPasswordAnswers.has(answer.toLowerCase())
        ? null
        : "A quick y or n will do.";
    case "password":
      return answer ? null : "Add a password or answer no to skip it.";
    default:
      return null;
  }
}
...
export function getDirectionTwoInlineFeedbackMessage(message) {
  ...
  if (lowerMessage.includes("15 minutes")) return "At least 15 minutes, please.";
  if (lowerMessage.includes("1 to 30")) return "Pick a number from 1 to 30.";
  if (lowerMessage.includes("only accepts numbers")) return "Numbers work best here.";
  if (lowerMessage.includes("answer y or n")) return "A quick y or n will do.";
...
export function getDirectionTwoCreateInlineInputError(value) {
  ...
  if ((step === "expiry" || step === "limit") && !/^\d+$/.test(currentValue)) {
    return step === "expiry" ? "Use numbers for the room timer." : "Use numbers for the member limit.";
  }

  if (step === "password-choice" && !isPasswordChoicePrefix(currentValue)) {
    return "A quick y or n will do.";
  }

  return null;
}

export function getDirectionTwoCreateVisualSegments(value, passwordVisibleIndex) {
  ...
  const isEditingPassword = getDirectionTwoCreateEditingStep(value) === "password";
  const passwordSeparatorIndex = isEditingPassword ? suffix.lastIndexOf("/") : -1;
  if (passwordSeparatorIndex !== -1) {
    const beforePassword = suffix.slice(0, passwordSeparatorIndex + 1);
    const rawPassword = suffix.slice(passwordSeparatorIndex + 1);
    const leadingPasswordSpace = rawPassword.match(/^\s*/)?.[0] ?? "";
    const trailingPasswordSpace = rawPassword.match(/\s*$/)?.[0] ?? "";
    ...
    if (password) {
      return [
        ...
        { text: `${trailingTopicSpace}${beforePassword}${leadingPasswordSpace}`, tone: "normal" },
        { text: getDirectionTwoPasswordMask(password, passwordVisibleIndex), tone: "password" },
        { text: trailingPasswordSpace, tone: "normal" },
...
function parseSlashDelimitedCreateCommand(value) {
  ...
  if (!yesPasswordAnswers.has(normalizedPasswordChoice)) {
    return { status: "invalid", message: "a quick y or n will do" };
  }
...
function parseExpiry(value) {
  ...
  if (!Number.isInteger(expiry) || expiry < 15) {
    return {
      valid: false,
      message: "room time needs to be at least 15 minutes",
    };
  }
...
function parseRoomLimit(value) {
  ...
  if (!Number.isInteger(roomLimit) || roomLimit < 1 || roomLimit > 30) {
    return {
      valid: false,
      message: "choose a whole number from 1 to 30 for the member limit",
    };
  }
...
export function resolveDirectionTwoThemeChoice(value, random = Math.random) {
  ...
  if (trimmedValue === "5" || trimmedValue === "surprise" || trimmedValue === "surprise me") {
    const index = Math.max(0, Math.min(directionTwoThemePalette.length - 1, Math.floor(random() * directionTwoThemePalette.length)));
    return directionTwoThemePalette[index];
  }

  return directionTwoThemePalette.find(theme => theme.selection === trimmedValue) ?? null;
}
