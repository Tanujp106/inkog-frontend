export const commands = ["create", "join", "help", "command", "commands", "clear", "style", "sound"];
...
const inlineHintByCommand = {
  command: "tap enter to list commands",
  commands: "tap enter to list commands",
  help: "ask anything",
...
export function resolveDirectionTwoGhostTapCompletion(value, hasActiveFlow = false) {
  ...
  return completeDirectionTwoCommand(value) ?? resolveDirectionTwoEnterAction(value)?.value ?? completeDirectionTwoCreateField(value);
}
...
function parseInlineCommandPrefix(value) {
  const match = value.match(/^\s*\/(create|help|join|style|sound|command|commands|clear)(\s.*)?$/i);
  if (!match) return null;
...
export function parseDirectionTwoInlineCommand(value) {
  const match = value.match(/^\s*\/(create|help|join|style|sound|command|commands|clear)(?:\s+([\s\S]*))?$/i);
  if (!match) return null;
