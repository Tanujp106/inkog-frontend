...
test("custom 404 page mounts the pixel breakout game and keeps both home links", async () => {
  ...
  assert.match(game, /getBreakoutEnergyLevel/);
  assert.match(game, /getBreakoutColorStage/);
  assert.match(game, /hud\.score/);
  assert.match(game, /hud\.combo/);
  assert.match(game, /hud\.activeBalls/);
  assert.match(game, /single-player/i);
  assert.match(game, /for \(const \[index, ball\] of/);
  assert.doesNotMatch(game, /socket|multiplayer/i);
  ...
  assert.match(game, /createImpactFeedbackState/);
  assert.match(game, /getImpactTransform/);
  assert.match(game, /stepImpactFeedback/);
  assert.match(game, /triggerImpactFeedback/);
  assert.match(game, /const gameStageRef = useRef<HTMLDivElement \| null>\(null\)/);
  assert.match(game, /const feedbackRef = useRef\(createImpactFeedbackState\(\)\)/);
  assert.match(game, /for \(const impact of nextState\.impactEvents\)/);
  assert.match(game, /triggerImpactFeedback\(feedbackRef\.current, impact\)/);
  assert.match(
    game,
    /stepImpactFeedback\(\s*feedbackRef\.current,\s*seconds,\s*prefersReducedMotionRef\.current,?\s*\)/,
  );
  assert.match(game, /drawGame\([\s\S]*?feedbackRef\.current\.particles/);
  assert.match(game, /ref=\{gameStageRef\} className="not-found-breakout-stage"/);
  assert.match(notFoundStyles, /\.not-found-breakout-stage/);
  assert.match(notFoundStyles, /will-change:\s*transform/);
  assert.match(notFoundStyles, /\.not-found-breakout-ambient\s*\{/);
  assert.match(game, /<AmbientShaderBackground[\s\S]*?<div ref=\{gameStageRef\}/);
  assert.match(tick, /const shouldAnimateIdle =/);
  assert.match(tick, /mode === "idle" \|\| mode === "waiting"/);
  assert.match(
    tick,
    /else if \(shouldAnimateIdle\) \{[\s\S]*?advanceImpactFeedback\(deltaSeconds\);/,
    "idle and waiting frames should continue decaying active impact feedback",
  );
  assert.match(
    game,
    /\}, \[advanceGame, advanceImpactFeedback, applyImpactTransform, renderCurrentGame\]\);/,
    "the animation callback should retain its feedback dependency",
  );
  assert.match(tick, /renderCurrentGame\(idleBallOffset\)/);
  assert.match(
    game,
    /const handleRestart = useCallback\(\(\) => \{[\s\S]*?idleAnimationStartedAtRef\.current = window\.performance\.now\(\);[\s\S]*?setGame\(restartBreakout\(gameRef\.current\)\);/,
    "restarting from idle should restart the waiting-ball bounce phase",
  );
  assert.match(
    game,
    /const handleRestart = useCallback\(\(\) => \{[\s\S]*?feedbackRef\.current = createImpactFeedbackState\([\s\S]*?applyImpactTransform\(\);/,
    "restart should clear active shake and sparks immediately",
  );
  assert.match(
    game,
    /if \(document\.hidden\) \{[\s\S]*?feedbackRef\.current = createImpactFeedbackState\([\s\S]*?applyImpactTransform\(\);/,
    "backgrounding should clear active feedback before animation resumes",
  );
