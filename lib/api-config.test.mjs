...
import { getInkogApiBaseUrl, getInkogSocketBaseUrl } from "./api-config.mjs";
...
test("uses an explicitly configured Socket.IO URL", () => {
  assert.equal(
    getInkogSocketBaseUrl({
      nodeEnv: "production",
      configuredUrl: "https://backend-wip.example.test/",
    }),
    "https://backend-wip.example.test",
  );
});

test("derives the Socket.IO URL from the configured API URL", () => {
  assert.equal(
    getInkogSocketBaseUrl({
      nodeEnv: "production",
      configuredApiUrl: "https://backend-wip.example.test/api",
    }),
    "https://backend-wip.example.test",
  );
});
