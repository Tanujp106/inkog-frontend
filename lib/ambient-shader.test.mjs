...
test("keeps color transitions on a single mounted shader", () => {
  const componentSource = readFileSync(new URL("../components/ambient-shader-background.tsx", import.meta.url), "utf8");

  assert.equal(componentSource.includes("function ShaderLayer"), false);
  assert.equal(componentSource.includes("shaderLayers.overlay"), false);
  assert.equal((componentSource.match(/<GrainGradient/g) ?? []).length, 1);
});
...
test("balances shader sharpness with a bounded render budget", () => {
  assert.equal(ambientShaderConfig.minPixelRatio, 1);
  assert.ok(ambientShaderConfig.maxPixelCount > 2073600);
  assert.ok(ambientShaderConfig.maxPixelCount <= 2100000);
});

test("pauses the ambient shader while the document is hidden", () => {
  const componentSource = readFileSync(new URL("../components/ambient-shader-background.tsx", import.meta.url), "utf8");

  assert.match(componentSource, /visibilitychange/);
  assert.match(componentSource, /speed=\{isDocumentVisible \? ambientShaderConfig\.speed : 0\}/);
});
