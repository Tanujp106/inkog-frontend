...
test("guided create keeps one editable command line and restores the active flow after edits", async () => {
  const source = await readFile(path.join(cwd, "components/direction-two-shell.tsx"), "utf8");
  ...
  assert.doesNotMatch(source, /starting private room setup/);
  assert.doesNotMatch(source, /answer each prompt, or use:/);
});
...
test("guided create keeps the question pill and plane button active for every answer", async () => {
  ...
  assert.doesNotMatch(source, /activePromptPresentation/);
  assert.doesNotMatch(source, /text-\[var\(--color-signal\)\]">Create<\/span>/);
});

test("guided create keeps question copy out of the active answer input", async () => {
  const source = await readFile(path.join(cwd, "components/direction-two-shell.tsx"), "utf8");

  assert.match(source, /inputValue \|\| \(flow\?\.type === "create" \? "" : placeholderFor\(flow\)\)/);
});

test("guided create uses create as the chatbox prompt label", async () => {
  const source = await readFile(path.join(cwd, "components/direction-two-shell.tsx"), "utf8");

  assert.match(source, /function promptFor[\s\S]*?return "create";/);
});
