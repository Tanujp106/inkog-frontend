...
test("guided create keeps the answer controls active without a visible question card", async () => {
  const source = await readFile(path.join(cwd, "components/direction-two-shell.tsx"), "utf8");
  const styles = await readFile(path.join(cwd, "app/globals.css"), "utf8");

  assert.match(source, /const hasPromptMenu = isSlashMenuOpen;/);
  assert.match(
    source,
    /guidedCreateQuestion && \(\s*<p aria-live="polite" className="sr-only" role="status">\s*\{guidedCreateQuestion\}\s*<\/p>\s*\)/,
  );
  assert.doesNotMatch(source, /direction-two-guided-question-pill/);
  ...
  assert.doesNotMatch(styles, /direction-two-guided-question-in/);
