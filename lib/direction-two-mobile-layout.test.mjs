...
test("landing slash menu opens above the pinned prompt without shifting layout", async () => {
  const source = await readFile(path.join(cwd, "components/direction-two-shell.tsx"), "utf8");

  assert.match(source, /className="relative w-full shrink-0"/);
  assert.match(source, /direction-two-slash-menu[^"]*absolute[^"]*bottom-full[^"]*left-0[^"]*z-20/);
  assert.doesNotMatch(source, /direction-two-slash-menu mt-3/);
});

test("landing transcript scrolls to the newest output while the slash menu opens upward", async () => {
  const source = await readFile(path.join(cwd, "components/direction-two-shell.tsx"), "utf8");
  ...
  assert.match(source, /terminalOutput\.scrollTo\(\{\s*top: terminalOutput\.scrollHeight,/);
