...
test("landing terminal colors the root prompt marker with the accent color", async () => {
  ...
  assert.match(source, /activePrompt === "\$" \? \(\s*<span className="text-\[var\(--color-signal\)\]">\$<\/span>/);
});
...
test("landing slash menu overlays prompt without shifting layout", async () => {
  const source = await readFile(path.join(cwd, "components/direction-two-shell.tsx"), "utf8");

  assert.match(source, /className="relative w-full"/);
  assert.match(source, /direction-two-slash-menu[^"]*absolute[^"]*left-0[^"]*top-full[^"]*z-20/);
  assert.doesNotMatch(source, /direction-two-slash-menu mt-3/);
});

test("landing slash menu smooth-scrolls before it clips below 80vh", async () => {
  const source = await readFile(path.join(cwd, "components/direction-two-shell.tsx"), "utf8");

  assert.match(source, /const slashMenuRef = useRef<HTMLDivElement \| null>\(null\)/);
  assert.match(source, /const scrollAnimationRef = useRef<DirectionTwoScrollAnimation \| null>\(null\)/);
  assert.match(source, /allowReverse: true/);
  assert.match(source, /anchorRatio: slashCommandSuggestions\.length > 0 \? 0\.8 : 0\.85/);
  assert.match(source, /floatingHeight: slashMenuHeight/);
  assert.match(source, /requestAnimationFrame\(tick\)/);
  assert.match(source, /getDirectionTwoScrollAnimationTop/);
  assert.match(source, /Math\.abs\(nextScrollTop - window\.scrollY\) <= 1/);
  assert.doesNotMatch(source, /behavior: "smooth"/);
  assert.match(source, /ref=\{slashMenuRef\}/);
  assert.match(source, /slashCommandSuggestions\.length > 0 \|\| lines\.length > 0/);
});
...
test("landing slash menu selection immediately executes action commands", async () => {
  ...
  assert.match(source, /slashMenuImmediateCommands = new Set\(\["\/clear", "\/command", "\/commands"\]\)/);
