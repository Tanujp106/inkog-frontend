...
test("exposes the supported global theme choices", () => {
  assert.deepEqual(inkogThemeChoices.map(theme => theme.id), [
    "orange",
    "blue",
    "green",
    "purple",
...
test("resolves surprise choices through the provided random function", () => {
  assert.equal(resolveInkogThemeChoice("surprise", () => 0)?.id, "orange");
  assert.equal(resolveInkogThemeChoice("surprise", () => 0.5)?.id, "cyan");
  assert.equal(resolveInkogThemeChoice("surprise", () => 0.999)?.id, "lime");

  assert.equal(inkogThemeColors.green, "#2f7d50");
});
