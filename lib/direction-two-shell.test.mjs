...
test("autocompletes the style command from a partial input", () => {
  assert.equal(completeDirectionTwoCommand("st"), null);
  assert.equal(completeDirectionTwoCommand("/st"), "/style");
  assert.equal(completeDirectionTwoCommand("/comm"), null);
  assert.equal(completeDirectionTwoCommand("command"), null);
  assert.equal(completeDirectionTwoCommand("/commands"), null);
});

test("autocompletes known inline command options from their typed prefix", () => {
  assert.equal(completeDirectionTwoCommandArgument("/sound / o"), "/sound / on");
  assert.equal(completeDirectionTwoCommandArgument("/sound / of"), "/sound / off");
  assert.equal(completeDirectionTwoCommandArgument("/sound / s"), "/sound / status");
  assert.equal(completeDirectionTwoCommandArgument("/sound / on"), null);
  assert.equal(completeDirectionTwoCommandArgument("/style / "), "/style / 1");
  assert.equal(completeDirectionTwoCommandArgument("/style / 2"), null);
  assert.equal(completeDirectionTwoCommandArgument("/join / abc123"), null);
  assert.equal(completeDirectionTwoCommandArgument("/help / explain rooms"), null);
});

test("requires slash-prefixed top-level command autocomplete", () => {
  assert.equal(completeDirectionTwoCommand("cre"), null);
  assert.equal(completeDirectionTwoCommand("style"), null);
  assert.equal(completeDirectionTwoCommand("/cre"), "/create");
  assert.equal(completeDirectionTwoCommand("/st"), "/style");
});

test("suggests landing slash commands only after slash input", () => {
  assert.deepEqual(
    getDirectionTwoSlashCommandSuggestions("/").map(item => item.command),
    ["/create", "/join", "/help", "/clear", "/style", "/sound"],
  );
  assert.deepEqual(
    getDirectionTwoSlashCommandSuggestions("/st").map(item => item.command),
    ["/style"],
  );
  assert.deepEqual(getDirectionTwoSlashCommandSuggestions("st"), []);
  assert.deepEqual(getDirectionTwoSlashCommandSuggestions("/create"), []);
  assert.deepEqual(getDirectionTwoSlashCommandSuggestions("/style"), []);
  assert.deepEqual(getDirectionTwoSlashCommandSuggestions("/create / room"), []);
});

test("uses clear landing copy for core slash commands", () => {
  assert.deepEqual(
    getDirectionTwoSlashCommandSuggestions("/").map(item => ({ command: item.command, label: item.label })),
    [
      { command: "/create", label: "create a room" },
      { command: "/join", label: "join room using code" },
      { command: "/help", label: "ask anything" },
      { command: "/clear", label: "clear terminal" },
      { command: "/style", label: "change color" },
      { command: "/sound", label: "sound settings" },
    ],
  );
});

test("provides mobile-friendly titles for slash command pills", () => {
  assert.deepEqual(
    getDirectionTwoSlashCommandSuggestions("/").map(item => ({ title: item.title, label: item.label })),
    [
      { title: "Create", label: "create a room" },
      { title: "Join", label: "join room using code" },
      { title: "Help", label: "ask anything" },
      { title: "Clear", label: "clear terminal" },
      { title: "Style", label: "change color" },
      { title: "Sound", label: "sound settings" },
    ],
  );
});

test("builds and edits guided create command segments without changing later values", () => {
  const started = createDirectionTwoGuidedCommandSegments("/create");
  const completed = [
    ["topic", "Hello"],
    ["expiry", "60"],
    ["limit", "15"],
  ].reduce(
    (segments, [id, value]) => appendDirectionTwoGuidedCommandSegment(segments, id, value),
    started,
  );

  assert.equal(serializeDirectionTwoGuidedCommandSegments(completed), "/create / Hello / 60 / 15");
  assert.equal(
    serializeDirectionTwoGuidedCommandSegments(
      updateDirectionTwoGuidedCommandSegment(completed, "topic", "Updated room"),
    ),
    "/create / Updated room / 60 / 15",
  );
...
test("does not parse bare create commands in the landing terminal", () => {
  assert.deepEqual(parseDirectionTwoCreateCommand("create / dinner / 45 / 4 / n"), {
    status: "not-create",
  });
  assert.equal(resolveDirectionTwoEnterAction("create"), null);
});
...
test("suggests the next inline create field while typing", () => {
  ...
  assert.equal(getDirectionTwoCreateHint("/create Goa December"), "how many minutes should the room stay open?");
...
test("shortens validation messages for inline feedback", () => {
  assert.equal(getDirectionTwoInlineFeedbackMessage("Expiry must be 15 minutes or more."), "At least 15 minutes, please.");
  assert.equal(getDirectionTwoInlineFeedbackMessage("Member limit must be from 1 to 30."), "Pick a number from 1 to 30.");
  assert.equal(getDirectionTwoInlineFeedbackMessage("Total time only accepts numbers."), "Numbers work best here.");
...
test("builds ghost text for the next inline create field", () => {
  ...
  assert.equal(getDirectionTwoCreateGhostText("/create "), "/ what should we call the room?");
  assert.equal(getDirectionTwoCreateGhostText("/create Goa December"), " / how many minutes should the room stay open?");
...
test("masks passwords with only the active character visible", () => {
  assert.equal(getDirectionTwoPasswordMask("T"), "T");
  assert.equal(getDirectionTwoPasswordMask("TA"), "*A");
  assert.equal(getDirectionTwoPasswordMask("TANUJ"), "****J");
  assert.equal(getDirectionTwoPasswordMask("TANUJ", 0), "T****");
  assert.equal(getDirectionTwoPasswordMask("TANUJ", 2), "**N**");
});
...
test("builds inline hints for command prefixes without leaving the prompt line", () => {
  ...
  assert.equal(getDirectionTwoInlineGhostText("/help / "), "ask anything");
...
test("continues incomplete command prefixes inline on enter", () => {
  ...
  assert.deepEqual(resolveDirectionTwoEnterAction("/create Goa December"), {
    ...
    hint: "how many minutes should the room stay open?",
  });
  assert.deepEqual(resolveDirectionTwoEnterAction("/create / Goa December"), {
    type: "continue-inline",
    value: "/create / Goa December / ",
  ...
  assert.equal(resolveDirectionTwoEnterAction("/command"), null);
  assert.equal(resolveDirectionTwoEnterAction("/commands"), null);
...
test("parses slash-delimited arguments for non-create commands", () => {
  ...
  assert.equal(parseDirectionTwoInlineCommand("/commands / "), null);
...
test("continues slash-delimited create answers between questions", () => {
  ...
  assert.equal(getDirectionTwoInlineGhostText("/create / hi sdff room / "), "how many minutes should the room stay open?");
...
test("marks the room name and active password segments for themed inline create rendering", () => {
  ...
  assert.deepEqual(getDirectionTwoCreateVisualSegments("/create / hello room / 50 / 4 / y / secret"), [
    { text: "/create / ", tone: "normal" },
    { text: "hello room", tone: "topic" },
    { text: " / 50 / 4 / y / ", tone: "normal" },
    { text: "*****t", tone: "password" },
  ]);
  assert.deepEqual(getDirectionTwoCreateVisualSegments("/create / hello room / 50 / 4 / y / secret", 0), [
    { text: "/create / ", tone: "normal" },
    { text: "hello room", tone: "topic" },
    { text: " / 50 / 4 / y / ", tone: "normal" },
    { text: "s*****", tone: "password" },
  ]);
...
test("describes inline command prompt icons", () => {
  ...
  assert.equal(getDirectionTwoInlinePromptPresentation("/commands / "), null);
...
test("resolves mobile ghost-tap autocomplete without changing flow prompts", () => {
  assert.equal(resolveDirectionTwoGhostTapCompletion("/cre", false), "/create");
  assert.equal(resolveDirectionTwoGhostTapCompletion("/sound / o", false), "/sound / on");
  assert.equal(resolveDirectionTwoGhostTapCompletion("/create / dinner", false), "/create / dinner / ");
  assert.equal(resolveDirectionTwoGhostTapCompletion("/style / ", false), "/style / 1");
  assert.equal(resolveDirectionTwoGhostTapCompletion("cre", true), null);
});
