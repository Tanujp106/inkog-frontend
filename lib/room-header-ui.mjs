...
export function buildRoomShareMessage({ topic, url, password, hasPassword }) {
  const safeTopic = String(topic ?? "").trim() || "private room";
  const safeUrl = String(url ?? "").trim();
  const safePassword = String(password ?? "").trim();
  const passwordCopy = safePassword || (hasPassword ? "ask the room creator" : "none");
