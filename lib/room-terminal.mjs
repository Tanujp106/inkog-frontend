...
const BARE_UTILITY_COMMANDS = new Set(["share", "leave", "exit", "close", "help", "password"]);
...
function parsePoll(rawArgs) {
  ...
  if (!question || options.length < 2) {
    return {
      type: "invalid",
      message: "Add a question and at least two options to make a poll.",
    };
  }
...
export function parseRoomCommand(rawValue) {
  ...
  if (parsedCommand) {
    if ((name === "command" || name === "commands" || (BARE_UTILITY_COMMANDS.has(name) && name !== "help")) && args) {
      return {
        type: "unknown",
        command: value,
      };
