...
test("keeps the paddle and resting ball frozen outside the running state", () => {
  ...
  assert.strictEqual(movePaddle(idle, 1, 1, true), idle);
  assert.strictEqual(setPaddleFromPointer(idle, 700), idle);
  assert.strictEqual(movePaddle(cleared, -1, 1, true), cleared);
  assert.strictEqual(setPaddleFromPointer(cleared, 100), cleared);
});
