...
function withCluster(state, size = 2, ballCount = 1) {
  const cluster = Array.from({ length: size }, (_, index) => ({
    id: `cluster-${index}`,
    digit: 0,
    row: index,
    column: index,
    x: 398 + (index % 2) * 12,
    y: 242 + Math.floor(index / 2) * 12,
    width: 12,
    height: 12,
    isActive: true,
  }));
...
test("emits collision-surface impact metadata while preserving wall and paddle sound events", () => {
  const running = launchBreakout(createInitialBreakoutState());
  const wallResult = stepBreakout(
    { ...running, ball: { ...running.ball, x: running.ball.radius - 1, y: 400, vx: -360, vy: 0 } },
    1 / 60,
  );
  const rightWallResult = stepBreakout(
    { ...running, ball: { ...running.ball, x: running.width - running.ball.radius + 1, y: 400, vx: 360, vy: 0 } },
    1 / 60,
  );
  const topWallResult = stepBreakout(
    { ...running, ball: { ...running.ball, x: 400, y: running.ball.radius - 1, vx: 0, vy: -360 } },
    1 / 60,
  );
  const paddleX = running.paddle.x + running.paddle.width * 0.3;
  const paddleResult = stepBreakout(
    {
      ...running,
      ball: {
  ...
  assert.deepEqual(wallResult.events, ["wall"]);
  assert.deepEqual(wallResult.impactEvents, [
    { type: "wall", x: 0, y: 400, normalX: 1, normalY: 0 },
  ]);
  assert.deepEqual(rightWallResult.impactEvents, [
    { type: "wall", x: running.width, y: 400, normalX: -1, normalY: 0 },
  ]);
  assert.deepEqual(topWallResult.impactEvents, [
    { type: "wall", x: 400, y: 0, normalX: 0, normalY: 1 },
  ]);
  assert.deepEqual(paddleResult.events, ["paddle"]);
  assert.deepEqual(paddleResult.impactEvents, [
    { type: "paddle", x: paddleX, y: running.paddle.y, normalX: 0, normalY: -1 },
  ]);
});
...
test("emits brick, clear, and miss impact metadata in sound-event order without changing outcomes", () => {
  const running = launchBreakout(createInitialBreakoutState());
  const target = running.bricks[0];
  const cleared = strikeBrick(
    {
      ...running,
      bricks: running.bricks.map((brick, index) => ({ ...brick, isActive: index === 0 })),
    },
  ...
  const missed = stepBreakout(
    {
      ...running,
      ball: { ...running.ball, x: 240, y: BREAKOUT_HEIGHT + running.ball.radius + 1, vx: 0, vy: 360 },
    },
    1 / 60,
  );

  assert.deepEqual(cleared.events, ["brickA", "clear"]);
  assert.deepEqual(cleared.impactEvents, [
    { type: "brickA", x: target.x + target.width / 2, y: target.y + target.height, normalX: 0, normalY: 1 },
    { type: "clear", x: target.x + target.width / 2, y: target.y + target.height, normalX: 0, normalY: 1 },
  ]);
  assert.equal(cleared.mode, "cleared");
  assert.deepEqual(missed.events, ["miss"]);
  assert.deepEqual(missed.impactEvents, [
    { type: "miss", x: 240, y: BREAKOUT_HEIGHT, normalX: 0, normalY: 1 },
  ]);
  assert.equal(missed.mode, "waiting");
...
test("clears stale impact metadata alongside sound events", () => {
  const staleImpact = { type: "wall", x: 0, y: 100, normalX: 1, normalY: 0 };
  const running = { ...launchBreakout(createInitialBreakoutState()), events: ["wall"], impactEvents: [staleImpact] };
  const idle = { ...createInitialBreakoutState(), events: ["miss"], impactEvents: [staleImpact] };

  assert.deepEqual(movePaddle(running, 0, 0.1).events, []);
  assert.deepEqual(movePaddle(running, 0, 0.1).impactEvents, []);
  assert.deepEqual(launchBreakout(idle).events, ["launch"]);
  assert.deepEqual(launchBreakout(idle).impactEvents, []);
});

test("starts with a normal paddle, one local ball, and progression counters", () => {
  const state = createInitialBreakoutState();

  assert.equal(state.balls.length, 1);
  assert.equal(state.ball, state.balls[0]);
  assert.equal(state.score, 0);
  assert.equal(state.combo, 0);
  assert.equal(state.comboTimer, 0);
  assert.equal(state.paddleTier, 1);
  assert.equal(getPaddleTierLabel(0), "small");
  assert.equal(getPaddleTierLabel(1), "normal");
  assert.equal(getPaddleTierLabel(2), "wide");
  assert.equal(getBreakoutEnergyLevel(state), 0);
});

test("destroys a cluster, awards score and combo, widens the paddle, and spawns one bonus ball", () => {
  const running = launchBreakout(createInitialBreakoutState());
  const result = stepBreakout(withCluster(running, 2), 1 / 120);

  assert.equal(result.bricks.filter(brick => brick.isActive).length, 1);
  assert.equal(result.balls.length, 2);
  assert.equal(result.ball, result.balls[0]);
  assert.equal(result.score, 20);
  assert.equal(result.combo, 2);
  assert.equal(result.paddleTier, 2);
  assert.ok(result.paddle.width > running.paddle.width);
  assert.ok(result.events.includes("multiBall"));
  assert.equal(getBreakoutEnergyLevel(result), 1);
});

test("caps a large cluster burst at four total balls", () => {
  const running = launchBreakout(createInitialBreakoutState());
  const result = stepBreakout(withCluster(running, 4, 3), 1 / 120);

  assert.equal(result.balls.length, 4);
  assert.ok(result.balls.every(ball => ball.isActive));
  assert.equal(result.score, 80);
  assert.equal(result.combo, 4);
  assert.equal(getBreakoutEnergyLevel(result), 2);
});

test("only loses a life after every active ball misses", () => {
  const running = launchBreakout(createInitialBreakoutState());
  const survivingBall = {
    ...running.ball,
    id: "ball-0",
  ...
  const missedBonusBall = {
    ...running.ball,
    id: "ball-1",
    x: 300,
    y: running.height + running.ball.radius + 1,
    vx: 0,
    vy: 360,
    isActive: true,
  };
  const result = stepBreakout({
    ...running,
    balls: [survivingBall, missedBonusBall],
    ball: survivingBall,
    combo: 4,
    comboTimer: 1,
    paddleTier: 2,
  ...
  assert.equal(result.balls[0].id, "ball-0");
  assert.equal(result.combo, 0);
  assert.equal(result.paddleTier, 1);
  assert.equal(result.events.at(-1), "miss");
});

test("resets progression and multi-ball state on restart or a final life loss", () => {
  const running = launchBreakout(createInitialBreakoutState());
  const changed = {
    ...withCluster(running, 2, 2),
    score: 120,
    combo: 6,
    comboTimer: 1,
    paddleTier: 2,
  ...
  const restarted = restartBreakout(changed);
  const finalMiss = loseBreakoutLife(changed);

  for (const state of [restarted, finalMiss]) {
    assert.equal(state.mode, state === restarted ? "idle" : "idle");
