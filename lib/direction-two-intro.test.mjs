...
test("uses gentle shimmer keyframes for mark and highlight hover", () => {
  ...
  assert.doesNotMatch(css, /@keyframes direction-two-title-shimmer/);
  assert.match(css, /\.direction-two-mark-shimmering \.direction-two-mark-word \.direction-two-mark-pixel-active \{\s*animation: direction-two-highlight-soft-shimmer/s);
  assert.match(
    css,
  ...
  assert.match(
    css,
    /\.direction-two-mark-shimmering \.direction-two-mark-word \.direction-two-mark-pixel-active \{\s*animation: direction-two-highlight-soft-shimmer/s,
  );
  assert.match(shell, /if \(showIntro\) return/);
...
test("keeps Direction 2 shimmer defaults independent of DialKit", () => {
  ...
  assert.doesNotMatch(layout, /DialRoot/);
  assert.doesNotMatch(layout, /dialkit\/styles\.css/);
  assert.doesNotMatch(shell, /useDialKit/);
  assert.doesNotMatch(shell, /DialConfig/);
  ...
  assert.match(shell, /titleDurationMs: 380/);
  assert.match(shell, /titleDelayMaxMs: 400/);
  assert.match(shell, /titleBurstTailMs: 420/);
  assert.match(shell, /titlePeakBrightness: 1\.9/);
  assert.match(shell, /titleSettleBrightness: 0\.7/);
  assert.match(shell, /titleSignalRadius: 21/);
  assert.match(shell, /titleHaloRadius: 14/);
  assert.match(shell, /titleSignalOpacity: 41/);
  assert.match(shell, /titleHaloOpacity: 46/);
  assert.match(shell, /colorMixPercent: 38/);
  assert.match(shell, /titleColorMixPercent: 46/);
...
test("keeps direction two shimmer intense without scale or lift motion", () => {
  ...
  const markResolveStart = css.indexOf("@keyframes direction-two-mark-pixel-resolve");
  const markResolveEnd = css.indexOf("@keyframes direction-two-mark-shimmer", markResolveStart);
  const markResolveBlock = css.slice(markResolveStart, markResolveEnd);
  ...
  assert.doesNotMatch(markResolveBlock, /transform:/);
