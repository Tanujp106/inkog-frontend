...
test("keeps direction two mark hover smooth and distinct from small icon shimmer", () => {
  ...
  assert.ok(directionTwoMarkMotion.hoverShimmerMs <= 850);
  assert.ok(directionTwoMarkMotion.markHoverMaxDelayMs <= 320);
...
test("uses a smooth immediate color ramp for inkog mark hover", () => {
  ...
  assert.match(
    css,
    /6% \{\s*background-color: color-mix\(in srgb, var\(--foreground\) 92%, var\(--color-signal\) 8%\);/s,
  );
  assert.match(
    css,
    /20% \{\s*background-color: color-mix\(in srgb, var\(--foreground\) 76%, var\(--color-signal\) 24%\);/s,
  );
  assert.match(
    css,
    /38% \{\s*background-color: color-mix\(in srgb, var\(--foreground\) 48%, var\(--color-signal\) 52%\);/s,
  );
  assert.match(
    css,
