...
test("keeps direction two mark hover smooth and distinct from small icon shimmer", () => {
  ...
  assert.ok(directionTwoMarkMotion.hoverShimmerMs >= 1180);
  assert.ok(directionTwoMarkMotion.markHoverMaxDelayMs >= 600);
...
test("uses a smooth immediate color ramp for inkog mark hover", () => {
  ...
  assert.match(
    css,
    /18% \{\s*background-color: color-mix\(in srgb, var\(--foreground\) 90%, var\(--color-signal\) 10%\);/s,
  );
  assert.match(
    css,
    /38% \{\s*background-color: color-mix\(in srgb, var\(--foreground\) 72%, var\(--color-signal\) 28%\);/s,
  );
  assert.match(
    css,
    /58% \{\s*background-color: color-mix\(in srgb, var\(--foreground\) 46%, var\(--color-signal\) 54%\);/s,
  );
  assert.match(
    css,
