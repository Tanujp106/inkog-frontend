...
test("keeps direction two mark hover as a bright static magnetic highlight", () => {
  ...
  assert.match(css, /filter: brightness\(var\(--direction-two-title-hover-highlight-brightness/);
...
test("separates strict title formation from the one-shot sine shimmer", () => {
  ...
  const shimmerEnd = css.indexOf("@keyframes direction-two-highlight-soft-shimmer", formEnd);
  ...
  assert.doesNotMatch(shimmerBlock, /transform:|filter:|box-shadow:/);
...
test("keeps mark magnetism from rereading layout and repainting every hover frame", () => {
  ...
  assert.match(shell, /const markPixelCentersRef = useRef<MarkPixelRecord\[\]>\(\[\]\)/);
  assert.match(shell, /const markPixelGridRef = useRef<Map<string, MarkPixelRecord\[\]>>\(new Map\(\)\)/);
  assert.match(shell, /function getMarkMagnetCandidates\(/);
  ...
  assert.match(shell, /if \(offset\.x !== record\.offsetX \|\| offset\.y !== record\.offsetY\)/);
  ...
  assert.doesNotMatch(css, /data-mark-magnet-active="true"\] \.direction-two-mark-pixel-active \{[\s\S]*transition:\s*transform 72ms var\(--direction-two-title-hover-easing/);
});
