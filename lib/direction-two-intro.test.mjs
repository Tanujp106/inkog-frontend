...
test("uses gentle shimmer keyframes for mark and highlight hover", () => {
  ...
  assert.match(
    css,
    /\.direction-two-mark-shimmering \.direction-two-mark-symbol \.direction-two-mark-pixel-active \{\s*animation: direction-two-highlight-soft-shimmer/s,
  );
  assert.match(
    css,
...
test("wires Direction 2 mark shimmer to the plain DialKit panel", () => {
  const packageJson = readFileSync(new URL("../package.json", import.meta.url), "utf8");
  const layout = readFileSync(new URL("../app/layout.tsx", import.meta.url), "utf8");
  const dialRoot = readFileSync(new URL("../app/playground-dial-root/index.tsx", import.meta.url), "utf8");
  const shell = readFileSync(new URL("../components/direction-two-shell.tsx", import.meta.url), "utf8");
  const css = readFileSync(new URL("../app/globals.css", import.meta.url), "utf8");

  assert.match(packageJson, /"dialkit": "\^1\.3\.0"/);
  assert.match(layout, /import "dialkit\/styles\.css"/);
  assert.match(layout, /<PlaygroundDialRoot \/>/);
  assert.match(dialRoot, /import \{ DialRoot \} from "dialkit"/);
  assert.match(dialRoot, /pathname !== "\/playground"/);
  assert.match(shell, /import \{ useDialKit \} from "dialkit"/);
  assert.match(shell, /useDialKit\("Direction 2 shimmer"/);
  assert.match(shell, /id: "direction-two-shimmer"/);
  assert.match(shell, /persist: true/);
  assert.match(shell, /title: \{/);
  assert.match(shell, /duration: \[660, 520, 2600, 20\]/);
  assert.match(shell, /sweepDelay: \[280, 120, 1200, 10\]/);
  assert.match(shell, /burstTail: \[130, 0, 900, 10\]/);
  assert.match(shell, /colorMix: \[38, 0, 80, 1\]/);
  assert.match(shell, /colorMix: \[62, 0, 92, 1\]/);
  assert.match(shell, /burstTail: \[120, 0, 600, 10\]/);
  assert.match(shell, /titleDurationMs: shimmerDials\.title\.duration/);
  assert.match(shell, /titleDelayMaxMs: shimmerDials\.title\.sweepDelay/);
  assert.match(shell, /titleBurstTailMs: shimmerDials\.title\.burstTail/);
...
test("keeps direction two shimmer intense without scale or lift motion", () => {
  ...
  assert.ok(directionTwoMarkMotion.markHoverMaxDelayMs >= 120);
  assert.ok(directionTwoMarkMotion.highlightHoverMaxDelayMs >= 48);
  assert.match(shell, /peakBrightness: \[1\.16, 0\.8, 2, 0\.01\]/);
  assert.match(shell, /signalRadius: \[4, 0, 24, 1\]/);
  assert.match(shell, /haloRadius: \[11, 0, 40, 1\]/);
  assert.match(shell, /signalOpacity: \[38, 0, 100, 1\]/);
  assert.match(shell, /haloOpacity: \[46, 0, 100, 1\]/);
  assert.match(shell, /peakBrightness: \[1\.34, 0\.9, 2\.4, 0\.01\]/);
  assert.match(shell, /signalRadius: \[6, 0, 28, 1\]/);
  assert.match(shell, /haloRadius: \[16, 0, 48, 1\]/);
  assert.match(shell, /signalOpacity: \[58, 0, 100, 1\]/);
  assert.match(shell, /haloOpacity: \[64, 0, 100, 1\]/);
