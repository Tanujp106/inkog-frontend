...
test("exposes separate DialKit controls for Direction 2 icons and title shimmer", () => {
  const packageJson = readFileSync(new URL("../package.json", import.meta.url), "utf8");
  const layout = readFileSync(new URL("../app/layout.tsx", import.meta.url), "utf8");
  const shell = readFileSync(new URL("../components/direction-two-shell.tsx", import.meta.url), "utf8");
  const css = readFileSync(new URL("../app/globals.css", import.meta.url), "utf8");

  assert.match(packageJson, /"dialkit"/);
  assert.match(layout, /DialRoot/);
  assert.match(layout, /dialkit\/styles\.css/);
  assert.match(shell, /useDialKit\("Icons animation"/);
  assert.match(shell, /useDialKit\("INKOG title animation"/);
  assert.match(shell, /directionTwoIconDialConfig/);
  assert.match(shell, /directionTwoTitleDialConfig/);
