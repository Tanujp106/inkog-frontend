...
test("keeps direction two mark hover smooth and distinct from small icon shimmer", () => {
  ...
  assert.ok(directionTwoMarkMotion.markHoverMaxDelayMs <= 180);
  assert.ok(directionTwoMarkMotion.highlightHoverShimmerMs >= 520);
...
test("uses gentle shimmer keyframes for mark and highlight hover", () => {
  ...
  assert.match(
    css,
    /\.direction-two-mark:hover \.direction-two-mark-word \.direction-two-mark-pixel-active \{\s*animation: direction-two-highlight-soft-shimmer/s,
  );
  assert.match(
    css,
    /\.direction-two-mark:hover \.direction-two-mark-symbol \.direction-two-mark-pixel-active \{\s*animation: direction-two-highlight-soft-shimmer/s,
  );
