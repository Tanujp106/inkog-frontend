...
test("removes the DialKit authoring surface while keeping fixed title motion defaults", () => {
  ...
  assert.doesNotMatch(lockfile, /dialkit/);
  assert.doesNotMatch(layout, /DialRoot|dialkit/);
  assert.doesNotMatch(shell, /useDialKit|DialConfig|directionTwoTitleDialConfig|inkog-title-motion/);
