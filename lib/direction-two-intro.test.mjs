...
test("derives a deterministic sine crest from global title coordinates", () => {
  ...
  const crestDelay = getDirectionTwoSineShimmerDelay(
    1,
    4,
    5,
    5,
    directionTwoTitleMotionDefaults.shimmerSpreadMs,
    directionTwoTitleMotionDefaults.shimmerAmplitudeMs,
    directionTwoTitleMotionDefaults.shimmerFrequency,
  );
  const oppositeEdgeDelay = getDirectionTwoSineShimmerDelay(
    1,
    0,
    5,
    5,
  ...
  assert.ok(oppositeEdgeDelay - crestDelay >= 140);
});
...
test("keeps magnetic attraction local and caps its displacement", () => {
  ...
  assert.ok(
    getDirectionTwoMagnetOffset(
      ...
      directionTwoTitleMotionDefaults.magnetStrength,
      directionTwoTitleMotionDefaults.magnetMaxDisplacement,
    ).x >= 3,
  );
});
...
test("provides stable defaults for the title motion pipeline", () => {
  assert.deepEqual(directionTwoTitleMotionDefaults, {
    ...
    shimmerDurationMs: 360,
    shimmerSpreadMs: 780,
    shimmerAmplitudeMs: 170,
    shimmerFrequency: 1,
    shimmerColorMixPercent: 68,
    shimmerPeakBrightness: 1.85,
    shimmerGlowRadius: 16,
    shimmerGlowOpacity: 62,
    magnetRadius: 120,
    magnetStrength: 1.15,
