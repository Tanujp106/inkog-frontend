...
test("provides stable defaults for the title motion pipeline", () => {
  assert.deepEqual(directionTwoTitleMotionDefaults, {
    ...
    hoverShimmerDurationMs: directionTwoMarkMotion.hoverShimmerMs,
    hoverShimmerSpreadMs: directionTwoMarkMotion.markHoverMaxDelayMs,
    magnetRadius: 48,
    magnetStrength: 1,
...
test("keeps direction two mark hover smooth and distinct from small icon shimmer", () => {
  ...
  assert.match(
    css,
    /\.direction-two-mark\[data-mark-phase="interactive"\]:hover \.direction-two-mark-pixel-active[\s\S]*direction-two-mark-hover-shimmer/,
  );
});
...
test("mounts one development-only DialKit panel on the title motion defaults", () => {
  ...
  assert.match(shell, /hoverShimmer: \{/);
