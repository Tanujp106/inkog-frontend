...
test("rejects unsupported sound commands", () => {
  assert.deepEqual(parseSystemSoundCommand("/sound loud"), {
    type: "invalid",
    message: "Try /sound on, /sound off, or /sound status.",
  });
});
...
test("sound profile keeps short UI sounds above the audible floor", () => {
  ...
  assert.ok(systemSoundNames.includes("pollCreated"), "poll creation should have a dedicated room sound");
  assert.ok(systemSoundNames.includes("pollVoted"), "poll voting should have a dedicated room sound");
  assert.ok(systemSoundNames.includes("countdownWarning"), "five-minute warnings should have a dedicated room sound");
...
test("sender and receiver message sounds have distinct shapes", () => {
  ...
  assert.ok(sent.length >= 2, "sent message should be audible enough to register on keyboard submit");
...
test("room-specific poll and countdown sounds stay purposeful", () => {
  assert.ok(systemSoundSpecs.pollCreated.length >= 2);
  assert.ok(systemSoundSpecs.pollVoted.length >= 1);
  assert.ok(systemSoundSpecs.countdownWarning.length >= 2);
  assert.ok(systemSoundSpecs.countdownWarning[0].frequency >= 500);
});
...
test("Breakout cues are distinct, brief, and balanced", () => {
  const breakoutSoundNames = [
    "breakoutLaunch",
    "breakoutWall",
    "breakoutPaddle",
    "breakoutBrickA",
  ...
  for (const soundName of breakoutSoundNames) {
    assert.ok(systemSoundNames.includes(soundName), `${soundName} should be a playable system sound`);
    const peakGain = getSystemSoundPeakGain(soundName);
    assert.ok(peakGain >= 0.24 && peakGain <= 0.5, `${soundName} peak gain should stay balanced`);
  }

  assert.deepEqual(
    [
      systemSoundSpecs.breakoutBrickA[0].frequency,
      systemSoundSpecs.breakoutBrickB[0].frequency,
      systemSoundSpecs.breakoutBrickC[0].frequency,
    ],
    [640, 720, 810],
  );
  assert.equal(systemSoundSpecs.breakoutWall[0].frequency, 310);
  assert.equal(systemSoundSpecs.breakoutPaddle[0].frequency, 430);
  assert.ok(systemSoundSpecs.breakoutWall.every(spec => spec.duration < 0.08));
  assert.ok(systemSoundSpecs.breakoutPaddle.every(spec => spec.duration < 0.08));
  assert.ok(systemSoundSpecs.breakoutBrickA.every(spec => spec.duration < 0.08));
  assert.ok(systemSoundSpecs.breakoutBrickB.every(spec => spec.duration < 0.08));
  assert.ok(systemSoundSpecs.breakoutBrickC.every(spec => spec.duration < 0.08));
  assert.equal(systemSoundSpecs.breakoutMiss.length, 2);
  assert.ok(
    systemSoundSpecs.breakoutMiss[0].frequency > systemSoundSpecs.breakoutMiss[1].frequency,
    "miss should descend",
  );
