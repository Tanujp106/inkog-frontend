...
test("describes the room style prompt choices", () => {
  assert.equal(getRoomStylePrompt(), "style: 1 orange, 2 blue, 3 green, 4 purple, 5 surprise (12 colors)");
});
...
test("resolves surprise room style choices", () => {
  assert.equal(resolveRoomStyleSelection("surprise", () => 0.5).theme?.id, "cyan");
  assert.equal(resolveRoomStyleSelection("surprise", () => 0.999).theme?.id, "lime");
});
