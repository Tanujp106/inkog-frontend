...
test("reports transition readiness only after the interactive room bootstrap is complete", () => {
  assert.equal(isRoomReadyForHandoff({ stage: "loading", historyHydrated: false, socketJoined: false, passwordGate: false }), false);
  assert.equal(isRoomReadyForHandoff({ stage: "joined", historyHydrated: false, socketJoined: true, passwordGate: false }), false);
  assert.equal(isRoomReadyForHandoff({ stage: "joined", historyHydrated: true, socketJoined: false, passwordGate: false }), false);
  assert.equal(isRoomReadyForHandoff({ stage: "joined", historyHydrated: true, socketJoined: true, passwordGate: true }), false);
  assert.equal(isRoomReadyForHandoff({ stage: "password", historyHydrated: true, socketJoined: true, passwordGate: true }), false);
  assert.equal(isRoomReadyForHandoff({ stage: "expired", historyHydrated: true, socketJoined: true, passwordGate: false }), false);
  assert.equal(isRoomReadyForHandoff({ stage: "error", historyHydrated: true, socketJoined: true, passwordGate: false }), false);
  assert.equal(isRoomReadyForHandoff({ stage: "joined", historyHydrated: true, socketJoined: true, passwordGate: false }), true);
});
